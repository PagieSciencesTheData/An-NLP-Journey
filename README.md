...
👉 Click here to view the live Interactive Tableau Dashboard on Tableau Public
...
The Problem: "What's all this saying?"
The Solution: I applied Topic Modeling and K-Means Clustering to automatically summarize, group, and extract the clear signal from the noise.
The Problem: "This shouldn't occupy my time."
The Solution: Instead of a fragile, one-off script, I built a structured, reproducible pipeline that processes unstructured text systematically so the user never has to manually parse a sprawling spreadsheet again.
The Problem: "Make it readable with nice charts."
The Solution: The pipeline engineered the data down to low-dimensional spatial coordinates (
x
,
y
), transforming abstract textual patterns into an executive-ready, interactive Tableau dashboard for rapid visual consumption.
